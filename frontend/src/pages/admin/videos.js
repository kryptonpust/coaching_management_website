    ...
    <div style={{ maxWidth: "100%" }}>
      <MaterialTable
        ...
        data={async query => {
          const result = await netrequest(`
          query{
            videos(size: ${query.pageSize}, page: ${query.page}) {
              count
              page
              rows {
                id
          ...
          //   console.log(result)
          return {
            data: result.videos.rows,
            page: result.videos.page,
            totalCount: result.videos.count
          };
        }}
        editable={{
          Add: {
          ...
          onRowAdd: async newData => {
            const result = await netrequest(`
            mutation {
                editVideos( chapterid: ${newData.chapterid},title: "${newData.title}", file: "${newData.file}")
                {
                  title
                  file
                }
          ...
          onRowUpdate: async (newData, oldData) => {
            await netrequest(`
            mutation {
                editVideos(id: ${newData.id}, chapterid: ${newData.chapterid},title: "${newData.title}", file: "${newData.file}")
                {
                  title
                  file
                }
